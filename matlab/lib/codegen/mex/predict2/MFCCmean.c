              ...
              *m)
{
  ...
  m->size[1] = 12;
  emxEnsureCapacity_real_T(sp, m, i5, &g_emlrtRTEI);
  ...
  while (i <= path->size[0] - 1) {
    i5 = path->size[1];
    if (!(2 <= i5)) {
      emlrtDynamicBoundsCheckR2012b(2, 1, i5, &bc_emlrtBCI, sp);
    }
    ...
    if (!((loop_ub >= 1) && (loop_ub <= i5))) {
      emlrtDynamicBoundsCheckR2012b(loop_ub, 1, i5, &ac_emlrtBCI, sp);
    }
    ...
    for (i5 = 0; i5 < 12; i5++) {
      m->data[i + m->size[0] * i5] = 0.5 * P->data[(b_path + P->size[0] * i5) -
        1] + 0.5 * Q->data[(c_path + Q->size[0] * i5) - 1];
    }
